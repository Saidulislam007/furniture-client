...
import { Star, CheckCircle, Send, ArrowRight, Package, Loader2, MessageSquare } from 'lucide-react';
...
export default function ProductReviewsPage() {
  ...
  const triggerToast = (msg: string) => {
    setToastMessage(msg);
    setTimeout(() => setToastMessage(null), 3000); // ৩ সেকেন্ড পর হাওয়া করে দেয়
  };
  ...
  // 🚀 রিভিউ সাবমিট লজিক (`sendToReviewsBackend` ইমপ্লিমেন্টেশন সহ)
  const handleSubmit = async (e: React.FormEvent, productId: string, productName: string) => {
    ...
    try {
      ...
      if (success) {
        ...
        // 🍞 কাস্টম সিগনেচার টোস্ট মেসেজ ফায়ার হলো
    ...
    <main className="max-w-[1920px] mx-auto px-4 sm:px-8 lg:px-12 xl:px-16 py-8 md:py-10 mt-12 md:mt-0 w-full font-sans min-h-screen bg-transparent relative">
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-50 bg-stone-950 text-white px-4 py-3 border border-amber-500/30 rounded-sm shadow-xl flex items-center gap-2.5 max-w-sm"
          >
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 max-w-3xl text-left">
        ...
        <div className="border-b border-stone-200/60 pb-5">
          ...
          <p className="text-xs text-stone-400 mt-1">Share your spatial experiences and design feedback for purchased manifests.</p>
        </div>

        {/* ⏳ পাইপলাইন ডাটা লোড লোডার */}
        {isLoading ? (
          <div className="w-full h-48 flex flex-col items-center justify-center gap-2 border border-stone-200/60 bg-white rounded-sm">
          ...
          <div className="w-full py-16 text-center border border-dashed border-stone-300 bg-white rounded-sm px-4">
            <MessageSquare className="w-8 h-8 text-stone-300 mx-auto mb-3 stroke-1" />
          ...
          /* 🖥️ লাইভ ডাইনামিক রিভিউ লিস্ট কার্ডস */
                        ...
                        <form onSubmit={(e) => handleSubmit(e, product.id, product.name)} className="p-5 space-y-4 border-l-2 border-stone-950">
                          ...
                          <div className="flex justify-end">
                            <button
                              ...
                              className="h-10 px-5 bg-stone-950 text-white text-xs font-serif uppercase tracking-widest hover:bg-stone-800 transition-colors flex items-center justify-center gap-2 rounded-sm min-h-[40px] shadow-xs disabled:opacity-60 disabled:cursor-not-allowed"
                            >
